...
export default (options?: OptionsTypes): Plugin => {
  return {
    ...
    config: async (config, env) => {
      ...
      try {
        ssl = await devcert.certificateFor(
          options?.certificateDomain || ["localhost"]
        );
      } catch (err) {
        console.error("vite-plugin-http2", err);
      }
      if (ssl != null && ssl.cert.toString() && ssl.key.toString()) {
        return {
          server: {
            https: {
      ...
      console.warn(
        "[vite-plugin-http2]: sorry, devcert create certificate fail, you can pass ssl option to create http2 server"
      );
      return {};
    },
    configureServer: async (server) => {
      if (options?.proxy != null) {
        server.middlewares.use((req, res, next) => {
          // 如果有一个配置命中请求，进行转发处理
          if (options.proxy != null) {
            for (const regexp in options.proxy) {
              const proxyOptions = options.proxy[regexp];
